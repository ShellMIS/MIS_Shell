    ...
    public partial class OpexReport : Form
    {
        ...
        private void btn_import_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ImportToExcel.Export(dt, "OpexReport(" + ReportType + ")", "OpexReport(" + ReportType + ")", "", DateTime.Now.ToString());
        ...
        private void bindDataFour()
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                for (int j = 2; j < dt.Columns.Count; j++)
                {
                    ...
                    {
                        dt.Rows[i][j] = double.Parse(dt.Rows[i][j].ToString());
                    }
                    else
                    {
                        dt.Rows[i][j] = 0.00;
        ...
        private void OpexReport_Load(object sender, EventArgs e)
        {
            ...
            //comboBox1.SelectedValue = "CoCd";
            //comboBox1.SelectedItem = "cdNameEn";
        ...
        private void comboType_SelectedValueChanged(object sender, EventArgs e)
        {
            if (comboType.SelectedItem.ToString().Trim() != "System.Data.DataRowView" && comboType.SelectedItem.ToString().Trim() != "")
            {
                if (comboType.SelectedItem.ToString().Trim() == "费用汇总")//4,5 
                {
                    ...
                    this.comboTypeChild.Items.AddRange(new object[] { comboType.SelectedItem.ToString().Trim() + "按期间", comboType.SelectedItem.ToString().Trim() + "按公司", comboType.SelectedItem.ToString().Trim() + "按部门/油站" });
        ...
        private void btn_Select_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    #endregion
                    string cocdT5T8string = MIS_T5.cocdt5t8string;
                    ...
                    if (cocdT5T8string == "")
                    {
                        if (cbCoCd.CheckedItems.Count > 0)
                        {
                            ...
                            {
                                DataRowView dv = ((DataRowView)cbCoCd.CheckedItems[i]);
                                cocd = dv["CoCd"].ToString().Trim();
                    ...
                    string GroupTime = "";
                    if (comboboxgroup.SelectedIndex != 0 && comboboxgroup.SelectedItem != null)
                    {
                        GroupTime = comboboxgroup.SelectedItem.ToString();
                    }
                    if (comboTypeChild.SelectedItem.ToString().Trim() == "费用汇总按期间")//4
                    {
                        #region 以前
                        //if (cbCoCd.CheckedItems.Count > 0)//合资公司的搜索条件
                        //{

                        //if (MIS_T5.staticT5 == "" && MIS_T5.staticT8 == "")
                        //{
                        //    where = " and tp.CoCd in(''" + comboJV.SelectedValue.ToString().Trim() + "'')";
                        //}
                        ...
                        #endregion
                        //封装后，调用存储过程
                        dt = importdal.ImportOPETotalByDate(starTime, endTime, cocdT5T8string, "op.PLLine,op.OpexLine", "convert(int,op.PLType)", GroupTime);
                        #region 以前
                        //}
                        //else
                        //{
                        // dt = importdal.ImportOPETotalByDate(starTime, endTime, "");//cocd不选择，则条件对cocd没有限制
                    ...
                    else if (comboTypeChild.SelectedItem.ToString().Trim() == "费用汇总按公司")
                    {
                        dt = importdal.ImportOPETotalByDate(starTime, endTime, cocdT5T8string, "op.PLLine,op.OpexLine", "convert(int,op.PLType)", "公司");
                        if (dt.Rows.Count > 0)
                        {
                            bindDataFour();
                        }
                    ...
                    else if (comboTypeChild.SelectedItem.ToString().Trim() == "费用汇总按部门/油站")//5
                    {
                        ...
                        dt = importdal.ImportOPETotalByDepart(starTime, endTime, cocdT5T8string, "op.PLLine,op.OpexLine", "convert(int,op.PLType),op.OpexLine", 0);
                    ...
                    else if (comboTypeChild.SelectedItem.ToString().Trim() == "费用明细按期间")//1
                    {
                        ...
                        dt = importdal.ImportOPETotalByDate(starTime, endTime, cocdT5T8string, "op.PLLine,op.OpexLine,tp.[AccountCodeD],op.Account_Description", "op.PLType,tp.AccountCodeD ", GroupTime);
                    ...
                    else if (comboTypeChild.SelectedItem.ToString().Trim() == "费用明细按公司")
                    {
                        dt = importdal.ImportOPETotalByDate(starTime, endTime, cocdT5T8string, "op.PLLine,op.OpexLine,tp.[AccountCodeD],op.Account_Description", "op.PLType,tp.AccountCodeD ", "公司");
                    ...
                    {
                        dt = importdal.ImportOPETotalByDepart(starTime, endTime, cocdT5T8string, "op.PLLine,op.OpexLine, tp.[AccountCodeD],op.Account_Description", " op.PLType,tp.AccountCodeD", 0);
                    ...
                    else if (comboTypeChild.SelectedItem.ToString().Trim() == "部门油站/期间汇总")//3
                    {
                        ...
                        dt = importdal.ImportOPETotalByDepart(starTime, endTime, cocdT5T8string, "tp.T5,jd.DeptNamePinyin", " tp.T5", 1);
                        if (dt.Rows.Count > 0)
                        {
                            bindDataThree();
                        }
                    ...
                    if (comboTypeChild.SelectedItem.ToString().Trim() == "费用汇总按期间" || comboTypeChild.SelectedItem.ToString().Trim() == "费用汇总按部门油站/期间汇总" || comboTypeChild.SelectedItem.ToString().Trim() == "费用汇总按公司")
                    {
                    ...
                    if (comboTypeChild.SelectedItem.ToString().Trim() == "费用明细按期间" || comboTypeChild.SelectedItem.ToString().Trim() == "费用明细按部门油站/期间汇总" || comboTypeChild.SelectedItem.ToString().Trim() == "费用明细按公司")
                    {
                        dataGridView1.Columns[0].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
                        dataGridView1.Columns[1].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
                    ...
                    if (comboTypeChild.SelectedItem.ToString().Trim() == "部门油站/期间汇总")
                    {
                        dataGridView1.Columns[0].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
                        dataGridView1.Columns[1].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
                        dataGridView1.DefaultCellStyle.Format = "N2";
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnSelectT5andT8_Click(object sender, EventArgs e)
        {
            //获取选择的cocd       
            if (cbCoCd.CheckedItems.Count > 0)
            {
        ...
        private void comboTypeChild_SelectedValueChanged(object sender, EventArgs e)
        {
            if (comboTypeChild.SelectedItem.ToString().Trim() != "System.Data.DataRowView" && comboTypeChild.SelectedItem.ToString().Trim() != "")
            {
                if (comboTypeChild.SelectedItem != null)
                {
                    string SubType = comboTypeChild.SelectedItem.ToString();
                    if (SubType.Contains("按期间"))
                    {
