    ...
    public partial class JVSetting : Form
    {
        ...
        private void 删除ToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            else if (MessageBox.Show("确认删除选中的" + row2.ToString() + "条记录吗？", "请确认", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                if (DeleteResult)
                {
                    int result = logDal.OpertionLogInsert(userid, "JV Setting删除"+row2+"条记录操作", DateTime.Now.ToString(), "删除成功");
                    if (result < 0)
                    {
                ...
                {
                    int result = logDal.OpertionLogInsert(userid, "JV Setting删除" + row2 + "条记录操作", DateTime.Now.ToString(), "删除失败");
                    if (result < 0)
                    {
                        MessageBox.Show("日志插入失败！");
                        return;
        ...
        private void But_Insert_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (jvdal.JVInsert(CoCd, CoNameCH, CoNameEN, Share, group1, group2) == 1)
                {
                    ...
                    int result = logDal.OpertionLogInsert(userid, "JV Setting添加 "+CoCd+" 操作", DateTime.Now.ToString(), "添加成功");
                    if (result < 0)
                    {
                        MessageBox.Show("日志插入失败！");
                        return;
                ...
                {
                    MessageBox.Show("操作失败！");
                    int result = logDal.OpertionLogInsert(userid, "JV Setting添加 " + CoCd + " 操作", DateTime.Now.ToString(), "JV Setting添加失败");
                    if (result < 0)
                    {
                        MessageBox.Show("日志插入失败！");
                        return;
