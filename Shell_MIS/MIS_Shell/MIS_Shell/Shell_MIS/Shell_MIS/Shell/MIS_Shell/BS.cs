    ...
    public partial class BS : Form
    {
        ...
        private void BS_Load(object sender, EventArgs e)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    cbCoCd.Items.Add(dt.Rows[i][0].ToString().Trim());
                }
            }
        ...
        private void bindDataFour(DataTable dt)
        {
            ...
            else
            {
                ...
                for (int i = p; i > 0; i--)
                {
                    if (type == dt.Rows[i]["AccType"].ToString().Trim())
                    {
                        for (int k = 0; k < dt.Columns.Count; k++)
                        {
                            if (k >= 2)
        ...
        public DataTable MergeDataTable(DataTable dt1, DataTable dt2)
        {
            ...
            for (int i = 0; i < dt2.Columns.Count; i++)
            {
                ...
                {
                    if (!dt.Columns.Contains(dt2.Columns[i].ColumnName))
                    {
        ...
        /// 添加人：ydx
        /// 添加时间：2014-10-26
        /// 添加目的：合并Datatable 当年初期末按照公司查询的时候
        /// </summary>
        /// <param name="dt1"></param>
        /// <param name="dt2"></param>
        /// <returns></returns>
        //public DataTable MergeDataTableByMonthChuQiMo(DataTable dt1, DataTable dt2)
        //{
        //    //定义dt的行数   
        //    int dtRowCount = 0;
        //    //dt的行数为dt1或dt2中行数最大的行数   
        ...
        //        dt.Columns.Add(dt1.Columns[i].ColumnName);
        //    }
        //    //向dt中添加dt2的列名   ,如果存在cn12,则在该索引后添加一列
        ...
        private void btn_Select_Click(object sender, EventArgs e)
        {
            ...
            #region t8数值
            //t8 数值
            if (cbCoCd.CheckedItems.Count > 0)
            {
            ...
            //选择的cocds
            string cocds = "";
            for (int i = 0; i < cbCoCd.Items.Count; i++)
            {
                if (cbCoCd.GetItemChecked(i))
                {
                    if (cocds == "")
                    {
                        cocds = "" + cbCoCd.GetItemText(cbCoCd.Items[i]) + "";
                        if (t8 == "")
                        {
                            cocdt8string = cocdt8string + cbCoCd.GetItemText(cbCoCd.Items[i]).Substring(0, cbCoCd.GetItemText(cbCoCd.Items[i]).IndexOf(' ')) + ":null;";
                        }
                    }
                    else
                    {
                        cocds = cocds + "," + cbCoCd.GetItemText(cbCoCd.Items[i]) + "";
                        if (t8=="")
                        {
                            cocdt8string =cocdt8string+ cbCoCd.GetItemText(cbCoCd.Items[i]).Substring(0, cbCoCd.GetItemText(cbCoCd.Items[i]).IndexOf(' ')) + ":null;";
                        }
            ...
            //把cocd和t8所选择的数值，拼成"CN12:234,243;CN13:R34,R56;"
            string[] t8string = t8.Split(',');
            if (cocds == "")
            {
                cocdt8string = "";
            }
            else
            {
                if (t8 != "")
                {
                    for (int i = 0; i < t8string.Length; i++)
                    {
                        cocd = t8string[i].Substring(0, t8string[i].IndexOf('-'));//cn16
                        t81 = t8string[i].Substring(t8string[i].IndexOf('-') + 1);//T8 R60989
                        //如果不包含该cocd 则添加cocd
                        if (!cocdt8string.Contains(cocd))
                        {
                            cocdt8string += ";" + cocd + ":";
                            cocdt8string += t81 + ",";
                        }
                    ...
                    cocdt8string = cocdt8string.Substring(1, cocdt8string.Length - 2) + ";";
                }                
            }
            ...
            if (cocdt8string.Contains(",;"))
            {
                cocdt8string = cocdt8string.Replace(",;", ";");
            }
            ...
            else if (comb_BS.SelectedItem.ToString().Trim() == "差值")
            {
                ...
                DataTable dataChu = importdal.bsSeleByProcMonthChu(cocdt8string, AccountPeriod);//年初
                DataTable dtCha = importdal.bsSeleByProcMin(cocdt8string, AccountPeriod);//所选日期上个月的累计，当月的月发生额，
            ...
            else if (comb_BS.SelectedItem.ToString().Trim() == "年初期末")
            {
                ...
                {
                    MessageBox.Show("分组必须选择");
                    return;
                }
                DataTable dataChu = importdal.bsSeleByProcMonthChuByCoCd(cocdt8string, AccountPeriod);//年初
                DataTable dataMid = importdal.bsSeleByProc(cocdt8string, AccountPeriod);//从当年开始到本月的累计
                if (dataChu==null)
                {
                    if (dataMid!=null&&dataMid.Rows.Count>0)
                    {
                        if (dataMid.Columns.Count==2)
                        {
                             MessageBox.Show("没有数据");
                        return;
                        }
                        dataGridView1.DataSource = dataMid;
                        foreach (DataGridViewColumn item in dataGridView1.Columns)
                        {
                            if (item.Name.Trim().Contains("年初余额") || item.Name.Trim().Contains("期末余额"))
                            {
                ...
                else//datachu!=null
                {
                    if (dataMid!=null)
                    {
                        if (dataMid.Columns.Count == 2&&dataChu.Columns.Count==2)
                        {
                            MessageBox.Show("没有数据");
                            return;
                        }
                    ...
                    {
                        if (dataChu.Columns.Count==2)
                        {
                              MessageBox.Show("没有数据");
                            return;
                        }
                        else if (dataChu.Rows.Count>0&&dataChu.Columns.Count>2)
                        {
                            dataGridView1.DataSource = dataChu;
                            foreach (DataGridViewColumn item in dataGridView1.Columns)
                            {
                                if (item.Name.Trim().Contains("年初余额") || item.Name.Trim().Contains("期末余额"))
                                {
                ...
                if (dataChu != null && dataChu.Rows.Count > 0)
                {
                    if (dataMid != null && dataMid.Rows.Count > 0)
                    {
                        ...
                        //    dt = this.MergeDataTableByMonthChuQiMo(dataChu, dataMid);
                        dt.Columns.Remove("AccType2");
                        dt.Columns.Remove("AccSubType2");
                        //将资产负债表里 “非流动负债”，“流动负债”，“股东权益”类里的数据乘以（-1）
                        for (int i = 0; i < dt.Rows.Count; i++)
                        ...
                        //查找期间发生额中有没有对应的cocd，没有则加0，有则加对应的值
                        int chucolumns = dataChu.Columns.Count - 2;//期初的总列数，索引从chuColumns开始到1+chucolumns结束
                        int QiJianColumns = dataMid.Columns.Count - 2;//期间发生额的总列数，索引从2+chucolumns开始,索引到dt.columns.count-1结束
                        //本年累计+ 期初值
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {

                            for (int j = 2; j < chucolumns + 2; j++)
                            {
                                dt.Columns[j].ReadOnly = false;
                                //期初某一列
                                string Chucocd = dt.Columns[j].ColumnName.Substring(0, dt.Columns[j].ColumnName.IndexOf(' '));
                                if (dt.Columns.Contains(Chucocd + "-期末余额") || dt.Columns.Contains(Chucocd + " 期末余额"))
                                {
                                    //有
                                    int QimoColumnsIndex = -1;
                                    if (dt.Columns.Contains(Chucocd + "-期末余额"))
                                    {
                                        QimoColumnsIndex = dt.Columns.IndexOf(Chucocd + "-期末余额");//在期间发生额数据那几列中找
                                    }
                                    if (dt.Columns.Contains(Chucocd + " 期末余额"))
                                    {
                                        QimoColumnsIndex = dt.Columns.IndexOf(Chucocd + " 期末余额");
                                    }

                                    dt.Columns[QimoColumnsIndex].ReadOnly = false;
                                    string qijian = "", qichu = "";
                                    if (dt.Rows[i][QimoColumnsIndex] != null)
                                    {
                                        if (dt.Rows[i][QimoColumnsIndex].ToString().Trim() == "")
                                        {
                                            qijian = "0.00";
                                        }
                                        else
                                        {
                                            qijian = dt.Rows[i][QimoColumnsIndex].ToString().Trim();
                                        }
                                    }
                                    if (dt.Rows[i][j] != null)
                                    {
                                        ...
                                        {
                                            qichu = dt.Rows[i][j].ToString().Trim();
                                        }
                                    }
                                    dt.Rows[i][QimoColumnsIndex] = double.Parse(qichu) + double.Parse(qijian);
                                }
                                else
                                {
                                    //在期间发生额那几列中找不到cocd，则表示这个这个cocd没有期间发生额，显示期初余额

                                    dt.Columns.Add(Chucocd + " 期末余额", typeof(double));
                                    if (dt.Rows[i][j] != null)
                                    {
                                        if (dt.Rows[i][j].ToString().Trim() == "")
                                        {
                                            dt.Rows[i][dt.Columns.Count - 1] = double.Parse("0.00");
                                        }
                                        else
                                        {
                                            dt.Rows[i][dt.Columns.Count - 1] = double.Parse(dt.Rows[i][j].ToString());
                                        }
                                    }


                                    //dt.Columns.Add(
                                    //dt.Rows[i][chuColumnsIndex] = double.Parse(dt.Rows[i][ChuI].ToString()) + double.Parse(dt.Rows[i][chuColumnsIndex].ToString());
                                }
                    ...
                    fieldMerge(dt);
                    createField(dt);
                    //格式化显示结果,查找期末余额那几列中对应cocd
                    DataTable dt2 = dt;
                    for (int i = dataChu.Columns.Count-1; i <dt2.Columns.Count; i++)
                    {
                        string QiMoName = dt2.Columns[i].ColumnName;//比如cn12-期末余额或者cn12期末余额
                        string cocd2 = "";
                        int chuindex = -1;
                        if (QiMoName.Contains('-'))
                        {
                            cocd2 = QiMoName.Substring(0, QiMoName.IndexOf('-'));
                            dt.Columns[i].ColumnName = cocd2 + " 期末余额";
                        ...
                        chuindex = dt.Columns.IndexOf(cocd2 + " 年初余额");
                        dt.Columns[i].SetOrdinal(chuindex + 1);
            ...
            #region 资产负债表类别为“ 按会计期间”

            else if (comb_BS.SelectedItem.ToString().Trim() == "按会计期间")
            {
                ...
                DataTable dataChu = importdal.bsSeleByProcMonthChu(cocdt8string, AccountPeriod);
                if (comboboxgroup.SelectedItem.ToString().Trim() == "按月")
                {
                    dataMonth = importdal.bsSeleByProcMonth(cocdt8string, AccountPeriod, "月");
                }
                else if (comboboxgroup.SelectedItem.ToString().Trim() == "按年")
                {
                    dataMonth = importdal.bsSeleByProcMonth(cocdt8string, AccountPeriod, "年");
                }
                else if (comboboxgroup.SelectedItem.ToString().Trim() == "按季度")
                {
                    dataMonth = importdal.bsSeleByProcMonth(cocdt8string, AccountPeriod, "季度");
                }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnSelectT8_Click(object sender, EventArgs e)
        {
            string cocds;
            ...
            {
                cocds = "";
                for (int i = 0; i < cbCoCd.CheckedItems.Count; i++)
                {
                    cocds += cbCoCd.CheckedItems[i].ToString().Trim().Substring(0, cbCoCd.CheckedItems[i].ToString().IndexOf(" ")) + ",";
                }
                //'cn13,cn14,cn15'
                string[] charcocds = cocds.Substring(0, cocds.Length - 1).Split(',');
                string aa = "','";
                cocds = string.Join(aa, charcocds);
                DataTable T8 = tcodeDal.TCodeSelectT8(cocds);
                if (T8 != null && T8.Rows.Count > 0)
                {
                    select_All.Checked = false;
                    checkedListBox.Items.Clear();
                    for (int i = 0; i < T8.Rows.Count; i++)
                    {
                        checkedListBox.Items.Add(T8.Rows[i][0].ToString().Trim());
                    }
                }
        ...
        private void comb_BS_SelectedValueChanged(object sender, EventArgs e)
        {
            if (comb_BS.SelectedItem.ToString().Trim() == "按会计期间")
            {
                lbGroupTime.Visible = true;
                comboboxgroup.Visible = true;
                this.comboboxgroup.Items.Clear();
                this.comboboxgroup.Items.AddRange(
    new object[] { "请选择", "按季度", "按月","按年" });
                this.comboboxgroup.SelectedIndex = 0;
            }
            else if (comb_BS.SelectedItem.ToString().Trim() == "年初期末")
            {
