...
namespace MIS_Shell
{
    public partial class MIS_Import : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (fd.ShowDialog() == DialogResult.OK)
            {
                ...
                if (!File.Exists(fileName))
                {
                    ...
                    #region 操作日志记录
                    int result = logDal.OpertionLogInsert(UserID, "MIS_Import数据包上传", DateTime.Now.ToString(), "文件不存在或已经删除");
                    if (result <= 0)
                    {
        ...
        private void bind(string fileName)
        {
            ...
            try
            {
                ...
                if (dtcompany.Rows[0][0].ToString().Trim() == "0")//合资公司
                {
                    ...
                    this.textBox1.Clear();
                    #region 操作日志记录
                    int result = logDal.OpertionLogInsert(UserID, "MIS_Import 选择公司与导入文件匹配", DateTime.Now.ToString(), "匹配失败！");
                    if (result <= 0)
                    {
                        MessageBox.Show("日志插入失败！");
                ...
                else if (dtDate.Rows[0][0].ToString().Trim() == "0")//会计期间
                {
                    ...
                    this.textBox1.Clear();
                    #region 操作日志记录
                    int result = logDal.OpertionLogInsert(UserID, "MIS_Import 期间与导入文件包匹配", DateTime.Now.ToString(), "匹配失败！");
                    if (result <= 0)
                    {
                        MessageBox.Show("日志插入失败！");
                ...
                else if (dtRepeat.Rows[0][0].ToString().Trim() != "0")
                {
                    ...
                    this.textBox1.Clear();
                    #region 操作日志记录
                    int result = logDal.OpertionLogInsert(UserID, "MIS_Import数据包上传", DateTime.Now.ToString(), "上传失败！");
                    if (result <= 0)
                    {
                        MessageBox.Show("日志插入失败！");
            ...
            catch (Exception err)
            {
                ...
                #region 操作日志记录
                int result = logDal.OpertionLogInsert(UserID, "MIS_Import数据包上传", DateTime.Now.ToString(), err.ToString());
                if (result <= 0)
                {
                    MessageBox.Show("日志插入失败！");
        ...
        private void but_jy_Click(object sender, EventArgs e)
        {
            ...
            #region 操作日志记录
            int result = logDal.OpertionLogInsert(UserID, "MIS_Import数据包匹配", DateTime.Now.ToString(), "匹配成功");
            if (result <= 0)
            {
                MessageBox.Show("日志插入失败！");
        ...
        private void but_Verify_Click(object sender, EventArgs e)
        {
            ...
            //T3的验证暂时不需要，如果需要的时候直接恢复492-506行的代码即可 2015-01-08
            //try
            //{
            ...
            if (dtCF.Rows.Count > 0)
            {
                ...
                //dataGridView2.Columns["Amount"].DefaultCellStyle.Format = "N2";
                //dataGridView2.Columns["Amount"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                #region 操作日志记录
                int result = logDal.OpertionLogInsert(UserID, "MIS_Import数据包验证", DateTime.Now.ToString(), "验证未通过！");
                if (result <= 0)
                {
                    MessageBox.Show("日志插入失败！");
            ...
            else
            {
                ...
                this.button1.Enabled = false;
                but_jy.Cursor = Cursors.Default;

                #region 操作日志记录
                int result = logDal.OpertionLogInsert(UserID, "MIS_Import数据包验证", DateTime.Now.ToString(), "通过验证");
                if (result <= 0)
                {
                    MessageBox.Show("日志插入失败！");
        ...
        private void btnDaoChu_Click(object sender, EventArgs e)
        {
            ...
            if (dtCF.Rows.Count <= 0)
            {
                ...
                int result = logDal.OpertionLogInsert(UserID, "MIS_Import 导出数据", DateTime.Now.ToString(), "没有数据可导出");
                if (result < 0)
                {
                    MessageBox.Show("日志插入失败！");
                    return;
            ...
            else
            {
                ...
                dtCF.Clear();
                int result = logDal.OpertionLogInsert(UserID, "MIS_Import 导出数据", DateTime.Now.ToString(), "导出成功");
                if (result < 0)
                {
                    MessageBox.Show("日志插入失败！");
                    return;
