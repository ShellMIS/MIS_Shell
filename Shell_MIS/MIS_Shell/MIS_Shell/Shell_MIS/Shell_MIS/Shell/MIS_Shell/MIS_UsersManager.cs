    ...
    public partial class MIS_UsersManager : Form
    {
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else if (MessageBox.Show("确认删除选中的" + row.ToString() + "条记录吗？", "请确认", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    #region 操作日志记录
                    int result = logDal.OpertionLogInsert(UserID, "Mis_UsersManager删除用户", DateTime.Now.ToString(), "删除成功");
                    if (result <= 0)
                    {
                ...
                {
                    #region 操作日志记录
                    int result = logDal.OpertionLogInsert(UserID, "Mis_UsersManager删除用户", DateTime.Now.ToString(), "删除失败");
                    if (result <= 0)
                    {
                        MessageBox.Show("日志插入失败！");
                ...
                try
                {
                    ...
                    if (userId > 0)
                    {
                        ...
                        if (uarDal.UandRInsert(userId.ToString(), roleId, cby, cdate, moby, modate) > 0)
                        {
                            ...
                            dataGridView1.DataSource = usDal.UserSelect();
                            #region 操作日志记录
                            int result = logDal.OpertionLogInsert(UserID, "Mis_UsersManager添加" + uname + "用户", DateTime.Now.ToString(), "添加成功");
                            if (result <= 0)
                            {
                                MessageBox.Show("日志插入失败！");
                    ...
                    else
                    {
                        ...
                        #region 操作日志记录
                        int result = logDal.OpertionLogInsert(UserID, "Mis_UsersManager添加" + uname + "用户", DateTime.Now.ToString(), "添加用户失败");
                        if (result <= 0)
                        {
                            MessageBox.Show("日志插入失败！");
                        }
        ...
        {
            if (this.dataGridView1.Columns[e.ColumnIndex].Name == "UserPwd" && e.Value != null && e.Value.ToString().Length > 0)
            {
