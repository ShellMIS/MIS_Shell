...
namespace DAL
{
    public class JVDSMDCDal
    {
        ...
        ///添加人“ydx
        ///添加时间：2014-12-29
        ///添加目的：根据合资公司查出此合资公司下的T5（Site部分）
        /// </summary>
        /// <param name="cocd"></param>
        /// <returns></returns>
        public DataTable getT5SiteByCocd(string cocd)
        {
            string sql = @"select  dep.T5Code, dep.T5Code+' '+dep.DeptNameCH as DeptNameCH  from JVDSMDC dep where   dep.CoCd='" + cocd + "' and dep.Nature='site' order by dep.T5Code";
        ...
        {
            string sql = "select dep.cocd+'-'+ltrim(rtrim(T5Code))+'-'+dep.DeptNameCH as DeptNameCH,dep.CoCd+'-'+T5Code as CocdT5Code  from JVDSMDC dep where   dep.CoCd in('" + cocd + "') and dep.T5Code<>''  group by CoCd,T5Code,DeptNameCH order by CoCd,T5Code";
            SqlDataReader dr = SqlHelp.ExecuteReader(sql);
            DataTable dt = new DataTable("jvdep");
            dt.Load(dr);
            dr.Close();
        ...
        {
            string sql = "select distinct dep.DeptNameCH,dep.T5Code  from JVDSMDC dep where  dep.CoCd='" + cocd + "' order by T5Code";
            SqlDataReader dr = SqlHelp.ExecuteReader(sql);
  ...
  public int JVDSMDCInsert(string DeptNameCH, string DeptNamePinyin, string Nature, string HSC, string CoCd, string T0Code, string T5Code, string T3Code, string SiteOpenDate, string SiteAging,
        ...
        public DataTable JVDSMDCSelectLike(string DeptNameCH, string DeptNamePinyin, string Nature, string HSC, string CoCd, string T0Code, string T3Code, string T5Code, string SiteOpenDate, string SiteAging,
      string SiteStatus, string TMCode, string Acquired, string Location, string CRType, string InvestmentType, string District)
        {
            SqlParameter[] parameters = new SqlParameter[17];
        ...
        {
            string sql = "select (CoCd+' '+CoNameEN)as cdNameEn,CoCd  from JVSetting where CoCd<>'' order by CoCd ";
            SqlDataReader dr = SqlHelp.ExecuteReader(sql);
