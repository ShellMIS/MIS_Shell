...
namespace DAL
{
    public class TCodeDal
    {
        ...
        public DataTable TCodeSelectT8Kpi(string Cocd)
        {
           // string sql = "select  CoCd+'-'+Tcode  as CocdT8Code from TCodeSetting where TcodeType='T8' and cocd in('" + Cocd + "')  group by cocd,Tcode order by cocd";
        ...
        {
            string sql = "select  CoCd+'-'+ltrim(rtrim(Tcode))   as CocdT8Code from TCodeSetting where TcodeType='T8' and cocd in ('" + Cocd + "')  group by cocd,TcodeName,Tcode order by cocd";
            //string sql = "select distinct Tcode from TCodeSetting where TcodeType='T8' and CoCd='"+Cocd+"' ";
            SqlDataReader dr = SqlHelp.ExecuteReader(sql);
            DataTable dt = new DataTable("t8");
            dt.Load(dr);
        ...
        /// 添加时间：2015-01-08
        /// </summary>
        /// <param name="TcodeType"></param>
        /// <param name="Tcode"></param>
        /// <param name="TcodeName"></param>
        /// <param name="CoCd"></param>
        /// <returns></returns>
        public DataTable TCodeSelectS(string TcodeType, string Tcode, string TcodeName, string CoCd)
        {
            string sql = @"select * from TCodeSetting where 1=1 ";
            if (!string.IsNullOrEmpty(TcodeType))
            {
                sql += "and TcodeType = '" + TcodeType + "'";
            }
