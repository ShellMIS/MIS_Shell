...
namespace DAL
{
   ...
   public  class KPIReportDAL
   {
       ...
       /// <param name="end"></param>
       /// <returns></returns>
       public DataTable getTotalSiteinOperation(string star, string end)
       {
           string sql = @"declare @sql varchar(max) select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),Period),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),Period),0,9))+''''+ 
            ' then [TotalSiteinOperation] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),Period),0,9))  
            from [T_TotalSite] bb where 1=1 and bb.Period >='" + star + "' and  bb.Period <='" + end + "'  GROUP BY SUBSTRING(CONVERT (varchar(10),Period),0,9)";
           sql += " select   @sql='select ''Total Site in Operation'' as  Type , '+@sql+' from T_TotalSite imp  '";
           sql += " EXEC(@sql)";
           SqlDataReader dr = SqlHelp.ExecuteReader(sql);
       ...
       /// <param name="end"></param>
       /// <returns></returns>
       public DataTable getNoofTEMPORARYCLOSED(string star, string end)
       {
           string sql = @"declare @sql varchar(max)  select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),Period),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),Period),0,9))+''''+ 
            ' then [TemporaryClosedSites] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),Period),0,9))  
            from [T_TotalSite] bb where 1=1 and bb.Period >='" + star + "' and  bb.Period <='" + end + "' GROUP BY SUBSTRING(CONVERT (varchar(10),Period),0,9)";
           sql += "  select   @sql=' select ''No. of TEMPORARY CLOSED'' as  Type , '+@sql+' from T_TotalSite imp  '";
           sql += " EXEC(@sql)";
           SqlDataReader dr = SqlHelp.ExecuteReader(sql);
       ...
       /// <param name="end"></param>
       /// <returns></returns>
       public DataTable getNoofNewOpenedSite(string star, string end)
       {
           string sql = @"declare @sql varchar(max) select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),Period),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),Period),0,9))+''''+ 
            ' then [NewOpenedSites] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),Period),0,9))  
            from [T_TotalSite] bb where 1=1 and bb.Period >='" + star + "' and  bb.Period <='" + end + "' GROUP BY SUBSTRING(CONVERT (varchar(10),Period),0,9)";
           sql += " select   @sql=' select ''No. of New Opened Site'' as  Type , '+@sql+' from T_TotalSite imp  '";
           sql += " EXEC(@sql)";
           SqlDataReader dr = SqlHelp.ExecuteReader(sql);
       ...
       /// <param name="end"></param>
       /// <returns></returns>
       public DataTable getNoofclosedSites(string star, string end)
       {
           string sql = @"declare @sql varchar(max) select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),Period),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),Period),0,9))+''''+ 
            ' then [closedSites] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),Period),0,9))  
            from [T_TotalSite] bb where 1=1 and bb.Period >='" + star + "' and  bb.Period <='" + end + "' GROUP BY SUBSTRING(CONVERT (varchar(10),Period),0,9)";
           sql += " select   @sql=' select ''No. of closed Sites'' as  Type , '+@sql+' from T_TotalSite imp  '";
           sql += " EXEC(@sql)";
           SqlDataReader dr = SqlHelp.ExecuteReader(sql);
       ...
       /// <param name="end"></param>
       /// <returns></returns>
       public DataTable getNoofNewSecuredSite (string star, string end)
       {
           string sql = @"declare @sql varchar(max) select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),Period),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),Period),0,9))+''''+ 
            ' then [NewSecuredSites] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),Period),0,9))  
            from [T_TotalSite] bb where 1=1 and bb.Period >='" + star + "' and  bb.Period <='" + end + "' GROUP BY SUBSTRING(CONVERT (varchar(10),Period),0,9)";
           sql += " select   @sql=' select ''No. of New Secured Site'' as  Type , '+@sql+' from T_TotalSite imp  '";
           sql += " EXEC(@sql)";
           SqlDataReader dr = SqlHelp.ExecuteReader(sql);
...
from [T_imp_raw] bb where 1=1 and  bb.AccountPeriod >='" + star + "' and  bb.AccountPeriod <='" + end + "' GROUP BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9) order by SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9) ";
           sql += "select @sql='select kp.ReportSubType as ''Type'','+@sql+' from KPISetting kp inner join T_imp_raw imp on imp.AccountCodeD=kp.AccountCode ";
...
from [T_imp_raw] bb where 1=1 and  bb.AccountPeriod >='" + star + "' and  bb.AccountPeriod <='" + end + "' GROUP BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)  ORDER BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9) ";
    ...
    sql+=" EXEC(@sql)";
       ...
       public DataTable getGENERALPRICEREDUCTIONS(string star, string end,string where)
       {
           ...
           sql+=" from KPISetting kp inner join T_imp_raw imp  ";
     ...
     /// <summary>
       ...
       /// <returns></returns>
       public DataTable getNETPROCEEDSOFSALESBasedonpumpprice_NFUEL_OTher(string star, string end, string where)
       {
           string sql = @"declare @sql varchar(max)";
           sql += "select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))+''''+ ";
           sql += " ' then [Amount] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))  ";
       ...
       /// <param name="end"></param>
       /// <returns></returns>
       public DataTable getC2TOC3COSTBYCATEGORY_storage(string star, string end, string where)
       {
           string sql = @"declare @sql varchar(max)";
           sql += "select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))+''''+ ";
           sql += " ' then [Amount] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))  ";
       ...
       /// <param name="where"></param>
       /// <returns></returns>
       public DataTable getTOTALSITEOPEXEXP_Other(string star, string end, string where)
       {
           //string sql = @"declare @sql varchar(max)";
           //sql += "select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))+''''+ ";
           //sql += " ' then [Amount] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))  ";
           ...
           //sql += " from KPISetting kp , T_imp_raw imp  where   imp.AccountCodeD like ''6%''   ";
           //sql += "    ";
           //if (where != string.Empty)
           //{
           //    sql += where;
           ...
           sql += " GROUP BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)  ORDER BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9) ";
           sql += "  select @sql='select kp.ReportGroup, kp.ReportType, '+@sql+'    ";
           sql += " from KPISetting kp inner join T_imp_raw imp  on   imp.AccountCodeD =kp.AccountCode   ";
           sql += "    ";
           if (where != string.Empty)
           {
               sql += where;
    ...
    /// <summary>
       ...
       /// <param name="end"></param>
       /// <returns></returns>
       public DataTable getTOTALJVSCLOVERHEADOther(string star, string end,string where)
       {
           string sql = @"declare @sql varchar(max)";
           sql += "select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))+''''+ ";
           sql += " ' then [Amount] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))  ";
          ...
          // sql += "  and kp.T2='''' ";
           ...
           //sql += " select @sql='select kp.ReportType as Type,'+@sql+' ";
           //sql += " from KPISetting kp inner join T_imp_raw imp   ";
           //sql += " on imp.AccountCodeD like (SUBSTRING(kp.AccountCode,1,1)+''%'')  ";
           //if (where != string.Empty)
           //{
           //    sql += where;
           //}
       ...
       /// <param name="end"></param>
       /// <returns></returns>
       public DataTable getOTHERINCOMEeXPENSE_TotalTMSLA(string star, string end,string where)
       {
           string sql = @"declare @sql varchar(max)";
           sql += "select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))+''''+ ";
           sql += " ' then [Amount] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))  ";
       ...
       public DataTable getC5BUSINESSCONTRIBUTION(string star, string end,string where)
       {
           ...
           sql += " from [T_imp_raw] bb where 1=1 and  bb.AccountPeriod >='" + star + "' and  bb.AccountPeriod <='" + end + "' ";
           sql += " GROUP BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)  ORDER BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9) ";
          // sql += " select @sql='select kp.Item, kp.ReportType as ReportSubType,'+@sql+' ";
           //select kp.Item, kp.ReportType as ReportSubType from KPISetting kp group by rollup(kp.Item, kp.ReportType)having kp.Item='CA'
          ...
          // sql += "  group by rollup(kp.Item, kp.ReportType)' ";
           sql += " EXEC(@sql) ";
           SqlDataReader dr = SqlHelp.ExecuteReader(sql);
       ...
       /// <param name="where"></param>
       /// <returns></returns>
       public DataTable getC5BUSINESSCONTRIBUTION_SITE(string star, string end, string where)
       {
           string sql = @"declare @sql varchar(max)";
           sql += "select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))+''''+ ";
           sql += " ' then [Amount] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))  ";
       ...
       /// <param name="where"></param>
       /// <returns></returns>
       public DataTable getC5BUSINESSCONTRIBUTION_JV(string star, string end, string where)
       {
           string sql = @"declare @sql varchar(max)";
           sql += "select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))+''''+ ";
           sql += " ' then [Amount] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))  ";
       ...
       /// <param name="where"></param>
       /// <returns></returns>
       public DataTable getC5BUSINESSCONTRIBUTION_IMPAIREMENT(string star, string end, string where)
       {
           string sql = @"declare @sql varchar(max)";
           sql += "select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))+''''+ ";
           sql += " ' then [Amount] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))  ";
       ...
       /// <param name="where"></param>
       /// <returns></returns>
       public DataTable getC5BUSINESSCONTRIBUTION_DISPOSAL(string star, string end, string where)
       {
           string sql = @"declare @sql varchar(max)";
           sql += "select  @sql=isnull(@sql+','+CHAR(15),'')+'sum(case when  SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)=' +''''+LTRIM(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))+''''+ ";
           sql += " ' then [Amount] else 0 end) as '+QUOTENAME(SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9))  ";
       ...
       public DataTable getFIN_HR_IT_LG(string star, string end,string where)
       {
           ...
           sql += " from [T_imp_raw] bb where 1=1 and  bb.AccountPeriod >='" + star + "' and  bb.AccountPeriod <='" + end + "' ";
           sql += " GROUP BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)  ORDER BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9) ";
           sql += " select @sql=' select kp.ReportType as ''Type'','+@sql+' from KPISetting kp inner join KPITemp kt on kp.Id=kt.Id inner join T_imp_raw imp on imp.T5 =kt.T5 and imp.AccountCodeD =kp.AccountCode  ";
           if (where != string.Empty)
           {
               sql += where;
           }
           ...
           //sql += " from [T_imp_raw] bb where 1=1 and  bb.AccountPeriod >='" + star + "' and  bb.AccountPeriod <='" + end + "' ";
           //sql += " GROUP BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)  ORDER BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9) ";
           //sql += " select @sql=' select kp.ReportType as ''Type'','+@sql+' from KPISetting kp inner join KPITemp kt on kp.Id=kt.Id inner join T_imp_raw imp on imp.T5 =kt.T5 and imp.AccountCodeD like (SUBSTRING(kp.AccountCode,1,1)+''%'') ";
           //if (where != string.Empty)
           //{
           //    sql += where;
           //}
       ...
       public DataTable getTAX(string star, string end,string where)
       {
           ...
           sql += " from [T_imp_raw] bb where 1=1 and  bb.AccountPeriod >='" + star + "' and  bb.AccountPeriod <='" + end + "' ";
           sql += " GROUP BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)  ORDER BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9) ";
           sql += "select @sql=' select kp.ReportType as ''Type'','+@sql+' from KPISetting kp inner join T_imp_raw imp on imp.AccountCodeD=kp.AccountCode ";
           if (where != string.Empty)
           {
               sql += where;
           }
       ...
       public DataTable getFINANCINGCOST(string star, string end,string where)
       {
           ...
           sql += " from [T_imp_raw] bb where 1=1 and  bb.AccountPeriod >='" + star + "' and  bb.AccountPeriod <='" + end + "' ";
           sql += " GROUP BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)  ORDER BY SUBSTRING(CONVERT (varchar(10),AccountPeriod),0,9)";
           sql += "select @sql=' select kp.ReportGroup as ''Type'','+@sql+' from KPISetting kp inner join T_imp_raw imp on imp.AccountCodeD=kp.AccountCode ";
           if (where != string.Empty)
           {
               sql += where;
           }
