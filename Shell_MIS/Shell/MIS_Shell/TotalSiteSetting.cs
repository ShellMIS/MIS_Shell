            ...
            if (comboCocd.SelectedValue.ToString()=="0")
            {
                ...
                else
                {
                    ...
                    if (dtExists.Rows.Count > 0)
                    {
                        ...
                        #region 操作日志记录
                        int result = logDal.OpertionLogInsert(UserID, "TotalSiteNumber 添加", DateTime.Now.ToString(), "此记录已存在");
                        if (result <= 0)
                        {
                            MessageBox.Show("日志插入失败！");
                    ...
                    else
                    {
                        ...
                        if (totalSiteId > 0)
                        {
                            ...
                            textUnopend.Clear();
                            #region 操作日志记录
                            int result = logDal.OpertionLogInsert(UserID, "TotalSiteNumber 添加", DateTime.Now.ToString(), "添加成功");
                            if (result <= 0)
                            {
                                MessageBox.Show("日志插入失败！");
                        ...
                        else
                        {
                            ...
                            #region 操作日志记录
                            int result = logDal.OpertionLogInsert(UserID, "TotalSiteNumber 添加", DateTime.Now.ToString(), "添加失败");
                            if (result <= 0)
                            {
                                MessageBox.Show("日志插入失败！");
                ...
                for (int i = dataGridView1.SelectedRows.Count; i > 0; i--)
                {
                    ...
                    if (Convert.ToBoolean(dataGridView1.Rows[rno].Cells[0].Value) == true)//全选的时候 删除最后一行会报错 如果不添加这行代码
                    {
                        ...
                        else
                        {
                            ...
                            #region 操作日志记录
                            int result = logDal.OpertionLogInsert(UserID, "TotalSiteNumber 删除", DateTime.Now.ToString(), "操作失败");
                            if (result <= 0)
                            {
                                MessageBox.Show("日志插入失败！");
